...
cmd:option('-result_folder', '', 'Result Folder')
...
local function eval_split(split, evalopt)
  ...
  local num_images = utils.getopt(evalopt, 'num_images', true)
    result_dir = utils.getopt(evalopt, 'result_folder', true)
  ...
  while true do
    ...
    for k=1,#sents do
      ...
      table.insert(predictions, entry)
        time = os.date("*t")
        entry.image_id = 'img_' .. time.hour .. time.min .. time.sec .. os.date ("_%A_%m_%B_%Y_") .. #predictions .. '.jpg'
        os.execute("mkdir " .. 'vis/img' .. time.hour .. time.min .. time.sec .. os.date ("%A%M%B%Y"))
      if opt.dump_images == 1 then
        -- dump the raw image to vis/ folder
        local cmd = 'cp "' .. path.join(opt.image_root, data.infos[k].file_path) .. '" vis/result/imgs/' .. entry.image_id -- bit gross
        print(cmd)
        os.execute(cmd) -- dont think there is cleaner way in Lua
      end
      if verbose then
        print(string.format('image %s: %s', entry.image_id, entry.caption))
        os.execute('curl --data "image=' .. entry.image_id .. '&caption=' .. entry.caption .. '" 0.0.0.0:5000/result')
      end
    end
...
if opt.dump_json == 1 then
  -- dump the json
  utils.write_json('vis/result/vis.json', split_predictions)
end
